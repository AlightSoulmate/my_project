  ...
  <div>
    <section>
      ...
      <div class="header-container">
        <el-button @click="isDialogVisible = true" type="primary">创建租户</el-button>
      ...
      <!-- 弹窗：创建租户 -->
      <el-dialog title="创建新租户" v-model="isDialogVisible" width="500" @close="resetDialogForm" style="z-index: 9999;">
        <el-form :model="newTenant">
          <el-form-item label="归属学院 (英)" required>
            <el-input v-model="newTenant.tenant_name" />
          </el-form-item>
        </el-form>
...
async function createNewTenant() {
  if (!newTenant.value.tenant_name) {
    return alert('请填写完整的信息')
  }

  const newData = newTenant.value

  const response = await createTenant(newData)
  if (response.ok) {
    alert('租户创建成功')
    newTenant.value = { tenant_name: '' } // 清空表单
    fetchTenants() // 重新获取
